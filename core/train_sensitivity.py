...
SCRIPT_VERSION = "hybrid-split-v7-sensitivity-nested-fpa"
...
def load_master_assignment_subset(
    ...
    *,
    ...
    features_per_assignment: int,
    subset_seed: int,
    source_features_per_assignment: Optional[int] = None,
    realization_subset_seed: int = 0,
) -> Tuple[nx.Graph, List[Dict[str, Any]], List[int]]:
    """Load nested assignment/FPA subsets from a saved master dataset.

    ``features_per_assignment`` is the requested FPA used for this sensitivity
    run. ``source_features_per_assignment`` is the FPA stored in the master
    simulation. When the requested FPA is smaller, a deterministic prefix of
    one fixed random permutation of master realization IDs is used. This makes
    the FPA sweep nested: FPA=5 contains the FPA=1 realization, FPA=10 contains
    the FPA=5 realizations, and so on.
    """
    if not os.path.isfile(graph_path):
        raise FileNotFoundError(f"Saved graph not found: {graph_path}")

    source_fpa = (
        int(features_per_assignment)
        if source_features_per_assignment is None
        else int(source_features_per_assignment)
    )
    target_fpa = int(features_per_assignment)
    if source_fpa < 1 or target_fpa < 1:
        raise ValueError("FPA values must be at least 1.")
    if target_fpa > source_fpa:
        raise ValueError(
            "features_per_assignment cannot exceed source_features_per_assignment."
        )

    local_rng = np.random.default_rng(realization_subset_seed)
    selected_local_ids = (
        local_rng.permutation(source_fpa)[:target_fpa].astype(int).tolist()
    )
    ...
    if os.path.isdir(shard_dir):
        ...
        for source_assignment_id in tqdm(
            ...
            if len(assignment_features) != source_fpa:
                raise ValueError(
                    ...
                    f"{len(assignment_features)} samples; expected source FPA "
                    f"{source_fpa}."
                )
            features.extend(assignment_features[index] for index in selected_local_ids)
    else:
        if not os.path.isfile(features_path):
            raise FileNotFoundError(
        ...
        expected_master = source_number_of_assignments * source_fpa
        ...
        for source_assignment_id in selected_ids:
            start = source_assignment_id * source_fpa
            features.extend(
                master_features[start + local_id] for local_id in selected_local_ids
            )
        del master_features
    ...
    print(f"Source FPA:                     {source_fpa}")
    print(f"Requested FPA:                  {target_fpa}")
    print(f"Realization subset seed:        {realization_subset_seed}")
    print(f"Selected source realization IDs:{selected_local_ids}")
...
def create_hybrid_split(
    *,
    ...
    if features_per_assignment < 1:
        raise ValueError("At least 1 realization per assignment is required.")
    ...
    if not 0.0 <= seen_validation_fraction < 1.0:
        raise ValueError(
            "seen_validation_fraction must be between 0 and 1, inclusive of 0."
        )
    ...
    if seen_validation_fraction > 0.0:
...
def validate_hybrid_split(split: Mapping[str, Any]) -> None:
    ...
    # Seen validation/test may be intentionally empty for FPA=1 sensitivity.
    required_nonempty = (
        "train",
        "validation_unseen",
        "test_unseen",
    ...
    train_membership = {sample_id // fpa for sample_id in split["train"]}
    if train_membership != development:
        raise ValueError("train does not match development assignments.")

    if split["validation_seen"]:
        assignment_membership = {
            sample_id // fpa for sample_id in split["validation_seen"]
        }
        if assignment_membership != development:
            raise ValueError("validation_seen does not match development assignments.")
...
def train_model_dual_validation(
    ...
    *,
    ...
    for epoch in range(num_epochs):
        ...
        seen_count = len(validation_seen_loader.dataset)
        unseen_count = len(validation_unseen_loader.dataset)
        if seen_validation_weight > 0.0 and seen_count == 0:
            raise ValueError(
                "seen_validation_weight is positive but validation_seen is empty."
            )
        if (1.0 - seen_validation_weight) > 0.0 and unseen_count == 0:
            raise ValueError(
                "Unseen-validation weight is positive but validation_unseen is empty."
            )

        validation_seen_loss = (
            _validation_loss(model, validation_seen_loader, device, criterion)
            if seen_count > 0
            else float("nan")
        )
        validation_unseen_loss = (
            _validation_loss(model, validation_unseen_loader, device, criterion)
            if unseen_count > 0
            else float("nan")
        )

        selection_loss = 0.0
        if seen_validation_weight > 0.0:
            selection_loss += seen_validation_weight * validation_seen_loss
        if seen_validation_weight < 1.0:
            selection_loss += (1.0 - seen_validation_weight) * validation_unseen_loss
...
def main(
    ...
    **model_kwargs: Any,
    ...
    source_number_of_assignments = int(source_number_of_assignments)
    if source_features_per_assignment is None:
        source_features_per_assignment = int(features_per_assignment)
    source_features_per_assignment = int(source_features_per_assignment)
    if source_features_per_assignment < int(features_per_assignment):
        raise ValueError(
            "source_features_per_assignment cannot be smaller than features_per_assignment."
        )
    if save_test_manifest and source_features_per_assignment != int(features_per_assignment):
        raise ValueError(
            "Test manifests are disabled for nested-FPA sensitivity runs because "
            "the exported local realization IDs would refer to the reduced subset. "
            "Use --save_test_manifest=False."
        )
    ...
    print(f"Requested FPA:               {features_per_assignment}")
    print(f"Master/source FPA:           {source_features_per_assignment}")
    print(f"Realization subset seed:     {realization_subset_seed}")
    ...
    graph_path, features_path = build_saved_paths(
        ...
        p2=p2,
        q_val=q_val,
        features_per_assignment=source_features_per_assignment,
    ...
    graph, features, selected_source_assignment_ids = load_master_assignment_subset(
        ...
        features_per_assignment=features_per_assignment,
        subset_seed=subset_seed,
        source_features_per_assignment=source_features_per_assignment,
        realization_subset_seed=realization_subset_seed,
    )
    ...
    for k in range(k_runs):
        ...
        validation_seen_eval: Optional[Dict[str, Any]] = None
        if len(loaders["validation_seen"].dataset) > 0:
        ...
        if validation_seen_eval is not None:
        ...
        if validation_seen_eval is not None:
            run_result["validation_seen"] = compact_metrics(validation_seen_eval)
