...
RESULT_ROOT="./results/sensitivity_cascades"
...
EXPECTED_TASKS=$(( ${#CASCADE_VALUES[@]} * NOISE_COUNT ))

for TASK_ID in $(seq 0 $((EXPECTED_TASKS - 1))); do

    echo "Task $TASK_ID: ${CASCADE_VALUES[$((TASK_ID / NOISE_COUNT))]} cascades, noise setting $((TASK_ID % NOISE_COUNT))"

    CASCADE_INDEX=$((TASK_ID / NOISE_COUNT))
