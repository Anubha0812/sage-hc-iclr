...
#SBATCH --job-name=fpa5k_test2
#SBATCH --error=err_%x_%a.txt
#SBATCH --output=out_%x_%a.txt
#SBATCH --ntasks=1
...
NOISE_COUNT=2
FPA_INDEX=$((SLURM_ARRAY_TASK_ID / NOISE_COUNT))
...
# Same master discovery rule as the user's FPA script: require a complete
# FPA=100, C=50 source master, preferring the 5000-assignment master over the 10000-assignment master.
P1_TAG=${P1//./}
P2_TAG=${P2//./}
MASTER_ASSIGNMENTS=""
MASTER_SHARD_DIR=""
for CANDIDATE in 5000 10000; do
...
if [ ! -f "$PROJECT_DIR/core/main_hybrid_split_v7_sensitivity_nested_fpa.py" ]; then
